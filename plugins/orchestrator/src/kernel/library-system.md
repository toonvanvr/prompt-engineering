...
> Skills source: `plugins/orchestrator/skills/`.
...
Skills follow the [Agent Skills](https://agentskills.io/) open standard. Source: `plugins/orchestrator/skills/` (VS Code native location via `chat.agentSkillsLocations`).

```
plugins/orchestrator/skills/{skill-name}/
...
**Startup:** Scan `plugins/orchestrator/skills/` for available skills (YAML frontmatter). Scan `.ai/library/` for patterns, domain knowledge, quirks.

**During Execution:** Task matches skill description → load from `plugins/orchestrator/skills/`. New knowledge discovered → write to `.ai/library/`. Before termination → update index.md.

## Cross-Tool Compatibility

|Tool|Skill Location|Format|
|-|-|-|
|VS Code Copilot|`plugins/orchestrator/skills/`|SKILL.md|
|Claude Code|`.claude/skills/`|SKILL.md|

## File Layout

No separate deploy step — compiled output is the deployed location.

|Path|Purpose|
|-|-|
|`plugins/orchestrator/agents/*.agent.md`|Agent definitions|
|`plugins/orchestrator/src/kernel/`|Behavioral rules|
|`plugins/orchestrator/skills/`|Reusable skills|
