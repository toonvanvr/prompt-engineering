...
|Design→Implementation|Spec exists + scope contained|Multiple domains AND public API change|
...
|Multi-domain changes|Multiple domains|Cross-cutting impact|
|Public interface changes|API/schema|Breaking change risk|

**Low-Risk Override:** Self-approve if ALL: few files, single domain, non-breaking, test coverage exists.
...
|Excessive file count|Spawn SA for partitioning|
