...
For each path containing `/` in `plugins/orchestrator/src/*.src.md`, verify it appears in corresponding `plugins/orchestrator/agents/*.agent.md`.
...
- Compare: `plugins/orchestrator/src/{agent}.src.md` §Kernel References → `plugins/orchestrator/agents/{agent}.agent.md` §Kernel References

### 4. Glossary Conformance
For each path defined in `plugins/orchestrator/src/kernel/glossary.md`, verify canonical form is used in kernel and shared files.
